...
class DeepSeekProvider(ProviderAdapter):
    """DeepSeek Chat Completions adapter in non-thinking mode."""
    ...
    def __init__(
        ...
        *,
        ...
        if config.provider is not ProviderName.DEEPSEEK:
            raise ProviderConfigurationError(
                "DeepSeekProvider requires deepseek configuration"
            )
        if not config.base_url:
            raise ProviderConfigurationError("DeepSeek API base URL is required")
        self.config = config
        self._client = client
        ...
        self._model_info = ModelInfo(
            ...
            display_name="DeepSeek Chat Completions API",
    ...
    async def health(self) -> ProviderHealth:
        ...
        return ProviderHealth(
            ...
            message=(
                ...
                if configured
                else "DeepSeek provider credentials are unavailable."
            ),
        )
    ...
    async def complete(self, request: CompletionRequest) -> CompletionResponse:
        ...
        attempt = request.metadata.get("provider_attempt")
    ...
    async def embeddings(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        raise ProviderConfigurationError("DeepSeek embeddings are not enabled")
    ...
    def _api_key(self, *, required: bool = True) -> str:
        ...
        if required and not value:
            raise ProviderConfigurationError("DeepSeek provider credentials are unavailable")
        return value

    def _validate_request(self, request: CompletionRequest) -> None:
        self.require_model(request.model)
        if any(message.images for message in request.messages):
            raise ProviderConfigurationError("DeepSeek text API does not support images")
    ...
    async def _post(
        ...
        try:
            ...
            raise ProviderTimeout("DeepSeek request timed out") from error
        except httpx.HTTPError as error:
            raise ProviderUnavailable("DeepSeek request failed") from error
        if response.status_code in {401, 403}:
            raise AuthenticationFailure("DeepSeek provider authentication failed")
        if response.status_code == 408:
            raise ProviderTimeout("DeepSeek request timed out")
        if response.status_code == 429:
            raise RateLimitExceeded("DeepSeek provider rate limit exceeded")
        if response.status_code != 200:
            raise ProviderUnavailable("DeepSeek provider rejected the request")
        return response
    ...
    def _response_body(response: httpx.Response) -> dict[str, Any]:
        try:
            ...
            raise ProviderUnavailable("DeepSeek provider returned invalid JSON") from error
        if not isinstance(body, dict):
            raise ProviderUnavailable("DeepSeek provider returned an invalid response")
        return body
    ...
    ) -> tuple[str, str, tuple[ProviderToolCall, ...]]:
        choices = body.get("choices")
        if not isinstance(choices, list) or not choices or not isinstance(choices[0], dict):
            raise ProviderUnavailable("DeepSeek provider returned no usable output")
        choice = choices[0]
        message = choice.get("message")
        if not isinstance(message, dict):
            raise ProviderUnavailable("DeepSeek provider returned no usable message")
        content = message.get("content") or ""
        if not isinstance(content, str):
            raise ProviderUnavailable("DeepSeek provider returned invalid content")
        tool_calls = cls._tool_calls(message.get("tool_calls"))
        finish_reason = choice.get("finish_reason")
        if finish_reason not in cls._finish_reasons:
            raise ProviderUnavailable("DeepSeek provider returned an invalid finish reason")
        if finish_reason == "tool_calls" and not tool_calls:
            raise ProviderUnavailable("DeepSeek provider returned empty tool calls")
        if finish_reason in {"stop", "length"} and not content.strip():
            raise ProviderUnavailable("DeepSeek provider returned empty output")
    ...
    def _tool_calls(raw: object) -> tuple[ProviderToolCall, ...]:
        ...
        if not isinstance(raw, list):
            raise ProviderUnavailable("DeepSeek provider returned malformed tool calls")
        result = []
        for item in raw:
            if not isinstance(item, dict) or not isinstance(item.get("id"), str):
                raise ProviderUnavailable("DeepSeek provider returned malformed tool call")
            function = item.get("function")
            if not isinstance(function, dict) or not isinstance(function.get("name"), str):
                raise ProviderUnavailable("DeepSeek provider returned malformed tool call")
            arguments = function.get("arguments", "{}")
            if not isinstance(arguments, str):
                raise ProviderUnavailable("DeepSeek provider returned malformed tool arguments")
            try:
                decoded = json.loads(arguments)
            except (TypeError, ValueError) as error:
                raise ProviderUnavailable("DeepSeek provider returned invalid tool arguments") from error
            if not isinstance(decoded, dict):
                raise ProviderUnavailable("DeepSeek provider tool arguments must be an object")
    ...
    def _message_payload(message: ProviderMessage) -> dict[str, object]:
        if message.role is MessageRole.TOOL:
            ...
            if result is None:
                raise ProviderConfigurationError("DeepSeek tool message is missing its result")
    ...
    def _usage(body: Mapping[str, Any]) -> TokenUsage:
        ...
        if not isinstance(usage, dict):
            raise ProviderUnavailable("DeepSeek provider returned invalid token usage")
